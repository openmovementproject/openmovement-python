"""OMX File Reader
Dan Jackson, Open Movement, 2017-2022

Derived from cwa_load.py CWA Reader by Dan Jackson, Open Movement.
"""
...
def _omx_parse_metadata(data):
    ...
    # OMX File has 448 bytes of metadata at offset 64
...
### !!! HACK: THIS IS ACTUALLY .CWA HEADER PARSING -- TODO: MODIFY FOR OMX
def _parse_omx_header(block):
    ...
    if len(block) >= 512:
        ...
        if packetHeader[0] == ord('M') and packetHeader[1] == ord('D') and packetLength >= 508:
            ...
            header['metadata'] = _omx_parse_metadata(block[64:512])     # @64  +448 "Annotation" meta-data (448 ASCII characters, ignore trailing 0x20/0x00/0xff bytes, url-encoded UTF-8 name-value pairs)
...
### !!! HACK: THIS IS ACTUALLY .CWA DATA PARSING -- TODO: MODIFY FOR OMX
def _parse_omx_data(block, extractData=False):
...
class OmxData(BaseData):
    """
    OMX file loader
    """

    def _read_data(self):
        if self.verbose: print('Opening OMX file...', flush=True)
    ...
    def _parse_header(self):
        ...
        self.header = _parse_omx_header(self.full_buffer)
        ...
        if (len(self.full_buffer) - self.data_offset >= SECTOR_SIZE):
            self.data_format = _parse_omx_data(self.full_buffer[self.data_offset:self.data_offset + SECTOR_SIZE])
    ...
    def _parse_data(self):
        ...
        self.np_data = np.frombuffer(self.data_buffer, dtype=dt_omx, count=-1)
    ...
    def __init__(self, filename, verbose=False, include_time=True, include_accel=True, include_gyro=True, include_mag=True, include_light=False, include_temperature=False):
        """
        Construct a OMX data object from a file.

        :param filename: The path to the .OMX file, it is memory-mapped so may be more efficient as a local file.
        ...
        ### !!! HACK: THIS IS ACTUALLY .CWA HEADER PARSING -- TODO: MODIFY FOR OMX
        raise Exception('.OMX parsing not yet fully implemented')
...
def _export(omx_data, filename):
    print('Exporting...')
    with open(filename, "wt") as fh:
        fh.write(','.join(omx_data.labels) + '\n')
        for row in omx_data:
...
def main():
    ...
    #_export(omx_data, os.path.splitext(filename)[0] + '.omx.csv')
